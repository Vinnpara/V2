...
void Vision::Road(int& hmin, int& hmax, int& smin, int& smax, int& vmin, int& vmax) {
	...
	if (vals.size() > 1 && stop_seen == false) {
		...
		if (v1 == 85 && v2 == 0) { 
			...
			putText(fr, "TRACKING LEFT", { 310,250 }, FONT_HERSHEY_PLAIN, 1, Scalar(255, 0, 0), 2);
		}
		...
		if (v1 == 180 && v2 == 85) { //Same as above but to steer right.
			...
			putText(fr, "TRACKING RIGHT", { 330,250 }, FONT_HERSHEY_PLAIN, 1, Scalar(0, 0, 255), 2);
		}
